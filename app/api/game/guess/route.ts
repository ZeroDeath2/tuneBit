...
import { processGuess } from "@/lib/game/game-engine";
...
export async function POST(request: Request) {
  try {
    ...
    const { sessionId, songId } = parsed.data;
    ...
    const { data: guessedSong } = await serviceClient
      .from("songs")
      .select("id, title, artist")
      .eq("soundcloud_id", songId)
      .single();
    ...
    const guessedSupabaseId = guessedSong.id;
    ...
    if (!correctSongId) {
      return NextResponse.json({ error: "Puzzle configuration error" }, { status: 500 });
    }
    ...
    const { newState, result } = processGuess(gameState, {
      songId: guessedSupabaseId,
      title: guessedSong.title as string,
      artist: guessedSong.artist as string,
      correctSongId,
    });
    ...
    await serviceClient.from("game_guesses").insert({
      ...
      guessed_song_id: guessedSupabaseId,
